...
"""
    ...
    Sorted By: Ref
    Fields: Item, Qty, Reference(s), Value, LibPart, Footprint, Datasheet, DNP, some additional symbol fields
...
def get_fields_avoid(path: str):
    fields_avoid = []
    try:
        with open(path) as fields_avoid_csv:
            fields_avoid_reader = csv.reader(fields_avoid_csv)
            for field_line in fields_avoid_reader:
                for field in field_line:
                    fields_avoid.append(field)
    except IOError:
        Warning()
...
if len(sys.argv) == 4:
    fields_avoid_csvpath = sys.argv[3]
...
fields_avoid = get_fields_avoid(fields_avoid_csvpath)
#fields_avoid = ['Arrow Part Number', ' Arrow Price/Stock', 'Height', 'Manufacturer_Name', 'Manufacturer_Part_Number', 'Mouser Part Number', 'Mouser Price/Stock']
columnset -= set(fields_avoid[:])
